...
import {
  ...
  deletePayRate,
  editPayRate,
} from "../../../services/payRatesServices";
import ModalPayRates from "./ModalPayRates";
import { emitter } from "../../../utils/emitter";
import ModalEditPayRates from "./ModalEditPayRates";
class ManagePaRates extends Component {
  ...
  handleDelete = async (user) => {
    try {
      let res = await deletePayRate(user.id);
  ...
  doEdit = async (user) => {
    try {
      let res = await editPayRate(user);
  ...
  render() {
    ...
    return (
      <div className="user-container">
        ...
        <div className="title text-center my-5">Manage Pay Rates</div>
        <div className="mx-1">
          <button
            className="btn btn-primary px-3 mx-5"
            onClick={() => this.handleAddNewUser()}
          >
            <i className="fas fa-plus"></i>Create new Pay Rates
          </button>
        </div>
        <div className="users-table mt-3 mx-5">
          <table className="table">
            <thead className="thead-light">
              <tr>
                <th scope="col">ID</th>
                <th scope="col">Pay Rate Name</th>
                <th scope="col">Value</th>
                <th scope="col">Tax Percentage</th>
                <th scope="col">Pay Type</th>
                <th scope="col">Pay Amount</th>
                <th scope="col">PT Level_C</th>
